...
<div class="container" style="margin-top: 100px; margin-bottom: 100px">
      <table class="tabelul table table-striped" style="width:100%;">
        ...
        <tbody>
  <?php
      $AllGames01 = $db->query('SELECT * FROM games ORDER BY id DESC');
      $AllGames = $AllGames01->fetchAll();

      foreach($AllGames as $key => $value)
      {
        echo '<tr><td>'.$value['id'].'</td>';
        if($value['cname'] && !$value['pname'])
        {
          echo '<td><img style="width: 32px; height: 32px;" src="'.$value['cavatar'].'"> '.$value['cname'].'</td>';
        }
        else
        {
          echo '<td><img style="width: 32px; height: 32px;" src="'.$value['cavatar'].'"> '.$value['cname'].' vs. <img style="width: 32px; height: 32px;" src="'.$value['pavatar'].'"> '.$value['pname'].'</td>';
        }
        if($value['cname'] && !$value['pname'])
        {
          echo '<td>$'.$value['ctp'].'</td>';
        }
        else
        {
          $sums = array($value['ctp'], $value['ptp']);
          $total = array_sum($sums);
          echo '<td>$'.$total.'</td>';
        }

        if($value['ended'] == '1' && $value['winner'] != '-1')
        {
          echo '<td>Ended, Winner player #'.$value['winner'].'</td>';
        }
        else if($value['winner'] != '-1' && $value['ended'] == '0')
        {
          echo '<td>Processing tradeoffer...</td>';
        }
        else if($value['winner'] != '-1' && $value['ended'] == '0' && $value['ptp'])
        {
          echo '<td>Waiting for user to join...</td>';
        }
        else
        {
          echo '<td>Waiting...</td>';
